"""Input validation utilities for benchmark problems and solvers.

Common validation functions with clear error messages for dimension mismatches,
invalid values, and other common mistakes.
"""
...
def check_matrix_dims(
    ...
    *,
    ...
    if len(A) != m:
        raise ValueError(
            f"Dimension mismatch: {name_b} has {m} constraints "
            f"but {name_A} has {len(A)} rows"
        )

    for i, row in enumerate(A):
        if len(row) != n:
            raise ValueError(
                f"Dimension mismatch: {name_c} has {n} variables "
                f"but {name_A} row {i} has {len(row)} columns"
            )
...
def check_sequence_lengths(
    *seqs: tuple[Sequence, str],
    ...
    for seq, name in seqs:
        if len(seq) != expected:
            raise ValueError(
                f"Length mismatch: expected {expected} elements in {name}, got {len(seq)}"
            )
...
def check_bounds(
    ...
    *,
    ...
    for i, (lo, hi) in enumerate(bounds):
        if lo > hi:
            raise ValueError(
                f"Invalid {name}[{i}]: lower bound {lo} > upper bound {hi}"
            )
    return n
...
def check_edge_nodes(
    ...
    *,
    ...
    for i, edge in enumerate(edges):
        ...
        if u < 0 or u >= n_nodes:
            raise ValueError(
                f"Invalid node in {name}[{i}]: u={u} (valid range: 0 to {n_nodes - 1})"
            )
        if v < 0 or v >= n_nodes:
            raise ValueError(
                f"Invalid node in {name}[{i}]: v={v} (valid range: 0 to {n_nodes - 1})"
            )
    ...
    *,
    ...
    for idx in integers:
        ...
        if idx < 0 or idx >= n_vars:
            raise ValueError(
                f"Invalid index in {name}: {idx} (valid range: 0 to {n_vars - 1})"
            )
